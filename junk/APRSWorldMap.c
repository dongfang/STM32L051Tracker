...
static const APRSPolygonVertex_t core145175[] = CORE_145175;
// 145010 has no core
// 145525 has no core
// 144930 has no core
...
static int16_t getLon(APRSPolygonVertex_t packed) {
	return packed / 181;
}

static int16_t getLat(APRSPolygonVertex_t packed) {
	uint16_t inter = packed % 181;
	return inter - 90;
}
...
static boolean checkWithinPolygon(int16_t lat, int16_t lon,
	...
	int16_t initialLat = getLat(boundaryList[*index]);
	int16_t initialLon = getLon(boundaryList[*index]);
	boolean result = true;
	// check for all segments AB in clockwise direction that angle (AB, AP) is positive
	do {
		int16_t previousLon = getLon(boundaryList[*index]);
		int16_t previousLat = getLat(boundaryList[*index]);
		// trace_printf("Trying: lat:%d, lon:%d @ index %d\n", previousLat, previousLon, *index);
		(*index)++;
		int ap0 = lon - previousLon;
		int ap1 = lat - previousLat;
		int ab0 = getLon(boundaryList[*index]) - previousLon;
		int ab1 = getLat(boundaryList[*index]) - previousLat;
		int cross = ap0 * ab1 - ap1 * ab0;
		if (cross < 0) {
			result = false;
		}
	} while (getLat(boundaryList[*index]) != initialLat
			|| getLon(boundaryList[*index]) != initialLon);
...
boolean checkWithinRegion(int16_t lat, int16_t lon,const APRSFrequencyRegion_t* region) {
	...
	while (!within
			&& region->boundary[vertexIndex] != POLYGON_LIST_END) {
...
boolean checkWithinCore(int16_t lat, int16_t lon, const APRSFrequencyRegion_t* region) {
	...
	uint16_t vertexIndex = 0;
	while (!within
			&& region->core[vertexIndex] != POLYGON_LIST_END) {
...
void checkWorldMap_Orientation_Polygon(const APRSPolygonVertex_t* boundaryList,
	...
	do {
		...
		int16_t previousLat = getLat(boundaryList[*index]);
		(*index)++;
		int16_t nextLonVector = getLon(boundaryList[*index]) - previousLon;	// n to n+1
		int16_t nextLatVector = getLat(boundaryList[*index]) - previousLat;
...
void APRS_checkWorldMapBoundaries_convex_polygons() {
	...
	for (int i = 0; i < APRS_WORLD_MAP_LENGTH; i++) {
		...
		//trace_printf("Trying region: %u\n", region->frequency);
		while (region->boundary[vertexIndex] != POLYGON_LIST_END) {
...
void APRS_checkWorldMapCoreAreas_convex_polygons() {
	...
	for (int i = 0; i < APRS_WORLD_MAP_LENGTH; i++) {
		...
		if (region->core == 0)
			continue;
		while (region->core[vertexIndex] != POLYGON_LIST_END) {
...
void APRS_checkCoreVertexExcursion() {
	...
	for (int i = 0; i < APRS_WORLD_MAP_LENGTH; i++) {
		...
		if (region->core == 0)
			continue;
		while (region->core[coreVertexIndex] != POLYGON_LIST_END) {
			// trace_printf("Trying poly starting at {%d,%d}\n",region->core[vertexIndex].lon,region->core[vertexIndex].lat);
			if (!checkWithinRegion(getLat(region->core[coreVertexIndex]),
					getLon(region->core[coreVertexIndex]), region)) {
				trace_printf("Excursing core vertex at {%d,%d}\n",
						getLon(region->core[coreVertexIndex]),
						getLat(region->core[coreVertexIndex]));
			}
			coreVertexIndex++;
		}
	}
