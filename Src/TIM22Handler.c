...
void TIM22_IRQHandler(void) {
	...
	if ((TIM22->SR & TIM_SR_UIF) && AFSKTransmitting) {
		...
		if (PT_captureCount >= 0) {
			/* Get the Input Capture value */
			char cse;
			uint32_t increment;
			if (captureValue >= PT_previousCaptureValue) {
				increment = captureValue - PT_previousCaptureValue;
			} else {
				increment = 60000UL + captureValue - PT_previousCaptureValue;
			}
			PT_extendedCaptureValue += increment;
		} else {
			// do nothing more.
			// trace_printf("first cap %d\n", captureValue);
		}

		PT_previousCaptureValue = captureValue;
		PT_captureCount++;
	}
}
